...
엄청나게 길어서 읽기도 힘든 논증이다. 이 논증의 진리표를 그리려면 우선 $P$, $Q$, $R$, $S$, $T$, $U$, $V$가 참/거짓인 경우를 각각 나눠야 하고, $\lnot P$, $Q \land R$, $Q \to (S \lor T)$ 와 ${(R \to U)}\allowbreak\to\nobreak({(S \to V)}\allowbreak\to\nobreak({(T \to V)}\allowbreak\to\nobreak({(\lnot P)}\allowbreak\to\nobreak{(U \land V)})))$이 각 경우에 참인지 거짓인지 계산해야하니, 무려 가로줄이 128 줄에 세로줄이 11 줄이고, 써야하는 진리값은 총 1508 개에 그 중 계산해야하는 진리값만 가로줄당 4 개씩 총 512 개가 된다. 말만 해도 끔찍하지 않은가?
...
진리표를 사용하지 않는 새로운 방식을 대략적으로 묘사해보자. 예를 들어 우리가

- ${P \to Q},\allowbreak{Q \to R}\allowbreak\vdash\nobreak{P \to R}$

을 증명하고 싶다고 할 때 다음과 같은 연역 과정(Deduction step)을 따를 수 있다.

1. $P \to Q$를 가정한다.
1. $Q \to R$를 가정한다.
1. $P$를 가정한다.
1. 3 번의 명제($P$)에 의해 1 번의 명제($P \to Q$)의 조건이 만족되므로 $Q$가 참이다.
1. 4 번의 명제($Q$)에 의해 2 번의 명제($Q \to R$)의 조건이 만족되므로 $R$이 참이다.
1. 5 번의 명제는 1, 2, 3 번의 전제를 가정해야 유도할 수 있다. 여기서 3 번의 전제를 조건으로 바꾸어 $P \to R$을 얻는다.
